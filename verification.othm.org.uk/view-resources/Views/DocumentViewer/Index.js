var DocumentViewerIndex = function () {
    ...
    const renderPage = function ($li) {
        ...
        config.isBlockchain = $li.data("isblockchain") === true || $li.data("isblockchain").toLowerCase() === "true";
        config.hasIssuance = $li.data("hasissuance") === true || $li.data("hasissuance").toLowerCase() === "true";
        ...
        switch (state) {
            case "NotFound":
                documentState += strFormat(dangerAlertTemplate, L("DocumentViewer:DocumentNotFound"));
                break;
            case "Expired":
                documentState += strFormat(dangerAlertTemplate, L("DocumentViewer:DocumentExpired", moment(expiryTime).format("L")));
                break;
            case "Valid":
                if (awardDate != null && awardDate != '') {
                    documentState += strFormat(successAlertTemplate, L("DocumentViewer:DocumentIssuedOn", issuingBody, studentFullName, moment(awardDate).format("L")));
                }
                else {
                    documentState += strFormat(successAlertTemplate, L("DocumentViewer:DocumentIssuedOnWithoutAwardDate", issuingBody, studentFullName));
                }
                break;
            case "Cancelled":
                documentState += strFormat(dangerAlertTemplate, L("DocumentViewer:DocumentCancelledOn", moment(cancelledDate).format("L")));
                break;
            case "OutOfDate":
                documentState += strFormat(warningAlertTemplate, L("DocumentViewer:DocumentOutOfDate"));
                break;
        }
        ...
        if (state !== "Expired" && state !== "NotFound") {
            ...
            let html = `<div id="documentViewerContainer" class="report-viewer-container mb-4">
                             <img src="/images/loading.gif" alt="${L("Loading")}" class="loader"/>
                         </div>`;
            ...
            html += `<div class="button-section">
                         <button id="verifyDocument" type="button" class="btn btn-info" aria-label="${L("DocumentViewer:VerifyButton", documentType)}" ${hasAccess ? "" : "disabled"}>
    ...
    const renderVerifyTable = function ($verifyButton) {
        ...
        for (let key in recordData) {
            ...
            if (value != null && value != '') {
                ...
                tableBody += `<tr>
                            <td>${key}</td>
                            <td class="hidden ${config.isBlockchain && !config.hasIssuance ? "table-warning-text" : "table-success-text"}">${value}</td>
                            <td class="hidden text-center">${config.isBlockchain && !config.hasIssuance
                        ? `<i class="far fa-hourglass-half table-warning-text fs-6" data-toggle="tooltip" data-placement="top" title="${L("DocumentViewer:VerificationTable:PendingTooltip")}"></i>`
                        : `<i class="far fa-check-circle table-success-text fs-6" data-toggle="tooltip" data-placement="top" title="${L("DocumentViewer:VerificationTable:VerifiedTooltip")}"></i>`}</td>
                        </tr>`;
            }
        }
    ...
    const getTableStatus = function () {
        if (config.isBlockchain) {
            if (!config.hasIssuance) {
                return strFormat(warningAlertTemplate, L("DocumentViewer:Blockchain:Status:Pending"));
            }

            return strFormat(successAlertTemplate, L("DocumentViewer:Blockchain:Status:Success"));
        }
