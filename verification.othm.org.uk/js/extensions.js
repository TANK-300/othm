...
$.validator.methods.required = function (value, element, param) {
    ...
    if (element.nodeName.toLowerCase() === "textarea" && element.classList.contains('summernote') && element.hasAttribute("required")) {
        let textOnly = $("<div>").html($(element).summernote("code")).text().trim();
...
function initDatepicker(container) {
    ...
    $(container).find('.disable-past-date').datetimepicker('setStartDate',new Date());
...
$('.m-timepicker,.kt-timepicker').timepicker({
    format: "hh:mm",
    showMeridian: true,
    todayHighlight: true,
    autoclose: true,
...
$('[maxlength]').not(".phone-number-input").maxlength({
    threshold: 10,
    placement: 'top-left',
...
function initValidation(formId, ruleSet, addEvents) {
    ...
    $form.submit(function (e) {
        ...
        const addressRegex = new RegExp(
            "[^a-zA-Z0-9\\-&\u0600-\u06ff\u0750-\u077f\ufb50-\ufbc1\ufbd3-\ufd3f\ufd50-\ufd8f\ufd50-\ufd8f\ufe70-\ufefc\uFDF0-\uFDFD\u0370-\u03ff\u1f00-\u1fff\u0400-\u04FFÀ-ÿœı '\\-,\"()\\\\/\\.]",
            "gm"
        );
...
function localeCurrencyFormat(num) {
    return abp.setting.get("App.System.Currency.Symbol") + num.toLocaleString(abp.localization.currentCulture.name, {
...
function LWithSource(sourceName, translationKey, ...args) {
    ...
    if (!source) return translationKey;
    try {
        return source(translationKey, ...args);
    } catch (e) {
...
var handleSelectAll = function () {
    switch ($(this).data("action")) {
        case "All":
            $('#' + $(this).data("forId")).find('option').prop('selected', 'selected').end().select2(selectAllConfig);
            $(this).data("action", "Deselect");
            $(this).text(L("Deselect"));
            break;

        case "Deselect":
            $('#' + $(this).data("forId")).find('option').prop('selected', false).end().select2(selectAllConfig);
            $(this).data("action", "All");
            $(this).text(L("SelectAll"));
            break;
    }
};
...
jQuery.validator.addMethod("greaterThan", function (value, element, params) {
    ...
    if (isNaN(value)) {
        ...
        const fromDate = moment(fromValue, dateFormat);
        const toDate = moment(value, dateFormat);
...
jQuery.validator.addMethod("lessThan", function (value, element, params) {
    ...
    if (isNaN(value)) {
        ...
        const toDate = moment(toValue, dateFormat);
        const fromDate = moment(value, dateFormat);
...
$("[data-datetime]").each(function () {
    ...
    const datetimeString = moment(datetime).local().format("L HH:mm:ss");
    $(this).text(datetimeString);
});
