...
var DocumentViewer = function () {
    ...
    const localisationSource = "DCWEB";
    ...
    function handleDocumentResult(result) {
        ...
        if (result && result.result) { setSessionDocuments(result.result.documents || []); }
    ...
    function renderDocument() {
        ...
        $.get({url: dataPath, dataType: "json"})
            ...
            .fail(function (response) {
                ...
                if (message === noAccessMessage && accessErrorModal) {
                    $('#PurchaseAccess').attr('href', '/store?documentId=' + session.recordId);
    ...
    function renderExpiryMessage() {
        const formattedExpiryDate = moment(session.expiryDate).format("L LTS");
